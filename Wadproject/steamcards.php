...
<html>
	<head>
		<title>STEAM WALLET CARDS-The Fuel</title>
		...
		function fun(x1,y1,id)
		{
		var parameters="price="+x1+"&name="+y1+"&id="+id;
				...
				<div style="display: inline-block;">
				  ...
				  <div style="display: inline-block;" class="menu-item"><li><a href="contact.php" class="menu-item2">Contact US</a></li></div>
				   <div style="display: inline-block;" class="menu-item"  style="width:120px;height:120px;"><li><a href="cart.php" class="menu-item2"><img src="empty-cart-light.png" alt="Shopping Cart" style="width:35px;height:35px;"><?php  if(!empty($_SESSION["cart"])){ echo "<span id='num'>".$_SESSION['count']."</span>";}else{ echo "<span id='num'></span>";}?></a></li></div>
					...
					<div id="third">
						<h1>STEAM WALLET CARDS<h1>
					</div>
					<div id="second">
						<ul class="menu2" class="ScrollStyle">
						<?php
						$con = new mysqli("localhost", "root", "", "wadproject");
						$q = "select * from giftcards where type='steam'";
						$rs = $con->query($q);
						while ( $r = $rs->fetch_assoc())
						{
							echo "<li><div class='img'>
							<img src='". $r["image"] ."' class='image'>
						  </br>". $r["name"] ."</br>RS.". $r["price"] ."</br><input type='button' value='ADD TO CART' class='bun' onclick='fun(\"". $r["price"] . "\",\"". $r["name"] . "\",\"". $r["id"] . "\");'>
						</div></li>";
